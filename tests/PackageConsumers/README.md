...
compilation prove that the integration package supplies the correct transitive `Reunion` and
`Reunion.Errors` dependencies and ASP.NET Core framework reference. Each web consumer compiles both
the `HttpResults` and MVC namespace-selected extension surfaces. All four consumers remain excluded
from `Reunion.slnx`.
...
package came from `artifacts/packages`. CI then builds and runs all four projects with `--no-restore`.

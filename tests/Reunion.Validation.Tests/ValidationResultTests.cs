...
public sealed class ValidationResultTests
{
    ...
    public void StatePropertiesAndOperations_RejectUninitializedDefault()
    {
        ...
        var operations = new Action[]
        {
            ...
            () => validation.Map(() => 1),
            () => validation.Map(() => 1, _ => "mapped"),
            () => validation.Bind(() => ValidationResult.Valid()),
            () => validation.Bind(() => UnitResult.Success<ValidationErrors>()),
            () => validation.Bind(() => Result.Success<int, ValidationErrors>(1)),
            () => validation.Bind(Result.Success, _ => "mapped"),
            () => validation.Bind(() => Result.Success(1), _ => "mapped"),
            () => validation.Bind(() => UnitResult.Success<string>(), _ => "mapped"),
            () => validation.Bind(() => Result.Success<int, string>(1), _ => "mapped"),
            () => validation.MapError(_ => CreateErrors(("name", "Mapped."))),
            () => validation.MapError(_ => "mapped"),
            () => validation.Tap(() => { }),
            () => validation.TapError(_ => { }),
            () => validation.Recover(_ => { }),
            () => validation.RecoverWith(_ => ValidationResult.Valid()),
